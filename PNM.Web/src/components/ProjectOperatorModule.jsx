...
const EMPTY_FORM = { projId:"", opId:"", allocationDate:TODAY, releaseDate:"", remarks:"" };

export default function ProjectOperatorModule() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [r, p, o, alloc] = await Promise.all([
        fetch(`${API}/ProjOpAllocation`, { headers:hdr() }).then(x=>x.json()),
        fetch(`${API}/Project`,          { headers:hdr() }).then(x=>x.json()),
        fetch(`${API}/Operator`,         { headers:hdr() }).then(x=>x.json()),
        fetch(`${API}/ProjOpAllocation`, { headers:hdr() }).then(x=>x.json()),
      ]);
      setRecords(r.data || []);
      setProjects(p.data || []);
      setOperators(o.data || []);
      setAllocations((alloc.data || []).filter(x => !x.releaseDate));
  ...
  const availableOperators = operators.filter(o =>
    !allocations.some(al => al.opId === o.opId) ||
    (editingId && allocations.find(al => al.projOpAllocId === editingId)?.opId === o.opId)
  );
  ...
  const openEdit = (r) => {
    setEditingId(r.projOpAllocId);
    setForm({ projId:String(r.projId), opId:String(r.opId), allocationDate:r.allocationDate||TODAY, releaseDate:r.releaseDate||"", remarks:r.remarks||"" });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = { projId:parseInt(form.projId), opId:parseInt(form.opId), allocationDate:form.allocationDate, releaseDate:form.releaseDate||null, remarks:form.remarks||null };
      const url    = editingId ? `${API}/ProjOpAllocation/${editingId}` : `${API}/ProjOpAllocation`;
  ...
  const handleDeallocate = async (id) => {
    if (!window.confirm("Deallocate this operator from the project?")) return;
    try {
      const res = await fetch(`${API}/ProjOpAllocation/${id}/deallocate`, { method:"PATCH", headers:hdr() });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`${API}/ProjOpAllocation/${id}`, { method:"DELETE", headers:hdr() });
      await load();
    } catch(e) { alert("Error: "+e.message); }
  };

  const filtered = records.filter(r =>
    (!filterProj   || String(r.projId) === filterProj) &&
    (!filterStatus || (filterStatus === "active" ? isActive(r) : !isActive(r)))
  );
  ...
  return (
    <div className="animate-fade">
      <div style={{ display:"flex", gap:"16px", marginBottom:"24px", flexWrap:"wrap" }}>
        {[
          { label:"Total Allocations", value:records.length,  icon:"ti-user",   color:"var(--primary-color)" },
      ...
      <div className="card">
        <div className="card-header" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
            ...
            <h5 style={{ margin:0, fontWeight:700, color:"var(--text-primary)" }}>Project Operator Allocation</h5>
            <span className="badge badge-pill badge-secondary" style={{ fontSize:"11px" }}>{filtered.length} records</span>
          </div>
          <button className="btn btn-carolina btn-sm" onClick={openAdd} id="btn-add-proj-op">
            <i className="ti-plus" style={{ marginRight:6 }} />Allocate Operator
          </button>
        </div>

        <div className="card-body">
          <div style={{ display:"flex", gap:"12px", marginBottom:"18px", flexWrap:"wrap" }}>
            <select className="form-control" style={{ flex:"1 1 180px", maxWidth:"240px" }} value={filterProj} onChange={e=>setFilterProj(e.target.value)} id="po-filter-proj">
              <option value="">All Projects</option>
              {projects.map(p=><option key={p.projId} value={p.projId}>{p.projName}</option>)}
            </select>
            <select className="form-control" style={{ flex:"1 1 140px", maxWidth:"180px" }} value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} id="po-filter-status">
          ...
          {loading ? (
            ...
            <div style={{ textAlign:"center", padding:"60px", color:"var(--text-secondary)" }}>
              ...
              <p style={{ fontWeight:600 }}>No operator allocations found.</p>
              <button className="btn btn-carolina btn-sm" onClick={openAdd} style={{ marginTop:8 }}><i className="ti-plus" style={{ marginRight:6 }} />Allocate First Operator</button>
            </div>
          ) : (
              ...
              <table className="table table-hover table-striped" style={{ marginBottom:0 }}>
                <thead>
                  <tr>
                    ...
                    <th>Project</th><th>Operator</th><th>Alloc. Date</th><th>Release Date</th><th>Status</th><th>Remarks</th>
                    <th style={{ width:120 }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((r,idx)=>(
                    <tr key={r.projOpAllocId}>
                      <td style={{ color:"var(--text-secondary)", fontSize:"12px" }}>{idx+1}</td>
                      <td style={{ fontWeight:600 }}>{r.projName}</td>
                      <td>{r.opFullName}</td>
                      <td style={{ fontSize:"12px" }}>{r.allocationDate}</td>
                      <td style={{ fontSize:"12px" }}>{r.releaseDate || <span style={{ color:"var(--text-secondary)", fontStyle:"italic" }}>Not released</span>}</td>
                      ...
                      <td>
                        <div style={{ display:"flex", gap:"4px" }}>
                          ...
                          {isActive(r) && <button className="btn btn-sm btn-outline-warning" style={{ padding:"3px 8px" }} title="Deallocate" onClick={()=>handleDeallocate(r.projOpAllocId)}><i className="ti-unlock" /></button>}
                          <button className="btn btn-sm btn-outline-danger" style={{ padding:"3px 8px" }} title="Delete" onClick={()=>handleDelete(r.projOpAllocId)}><i className="ti-trash" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {showModal && createPortal(
        <div className="modal fade show" style={{ display:"block", background:"rgba(0,0,0,0.55)", position:"fixed", inset:0, zIndex:1055, overflowY:"auto" }}
          ...
          <div className="modal-dialog" style={{ margin:"60px auto", maxWidth:"560px" }}>
            <div className="modal-content modal-content-custom">
              <div className="modal-header" style={{ borderBottom:"1px solid var(--border-color)", padding:"18px 24px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                  ...
                  <div>
                    <h5 className="modal-title" style={{ margin:0, fontWeight:700, color:"var(--text-primary)" }}>{editingId?"Edit Allocation":"Allocate Operator to Project"}</h5>
                    <p style={{ margin:0, fontSize:"12px", color:"var(--text-secondary)" }}>Assign an operator to a project. Only unallocated operators are shown.</p>
                  </div>
                </div>
              ...
              <div className="modal-body" style={{ padding:"24px" }}>
                <form onSubmit={handleSubmit} id="proj-op-form">
                  <SectionHeader label="Allocation Details" />
                  <FormRow>
                    <FormGroup label="Project" required>
                      <select className="form-control" name="projId" value={form.projId} onChange={handleChange} required id="po-proj">
                        <option value="">Select project…</option>
                        {projects.map(p=><option key={p.projId} value={p.projId}>{p.projName}</option>)}
                      </select>
                    </FormGroup>
                    <FormGroup label="Operator" required>
                      <select className="form-control" name="opId" value={form.opId} onChange={handleChange} required id="po-op">
                        <option value="">Select operator…</option>
                        {(editingId ? operators : availableOperators).map(o=><option key={o.opId} value={o.opId}>{o.fullName} ({o.opCode})</option>)}
                      </select>
                    </FormGroup>
                  </FormRow>
                  <FormRow>
                    <FormGroup label="Allocation Date" required>
                      <input type="date" className="form-control" name="allocationDate" value={form.allocationDate} onChange={handleChange} required id="po-alloc-date" />
                    </FormGroup>
                    <FormGroup label="Release Date">
                      <input type="date" className="form-control" name="releaseDate" value={form.releaseDate} onChange={handleChange} id="po-release-date" />
                    </FormGroup>
                  </FormRow>
                  <FormGroup label="Remarks">
                    <textarea className="form-control" name="remarks" rows={2} value={form.remarks} onChange={handleChange} placeholder="Optional remarks…" id="po-remarks" />
                  </FormGroup>
                  {!editingId && availableOperators.length === 0 && (
                    <div className="alert alert-warning" style={{ fontSize:"12px", marginTop:12 }}>
                      <i className="ti-alert" style={{ marginRight:6 }} />All operators are currently allocated. Deallocate one first.
                    </div>
                  )}
                </form>
              </div>
              <div className="modal-footer" style={{ borderTop:"1px solid var(--border-color)", padding:"14px 24px", display:"flex", justifyContent:"flex-end", gap:"10px" }}>
                ...
                <button type="submit" form="proj-op-form" className="btn btn-carolina" disabled={saving}>
                  {saving ? <><Loader2 size={14} style={{ animation:"spin 1s linear infinite", marginRight:6 }} />Saving…</> : <><i className="ti-save" style={{ marginRight:6 }} />{editingId?"Update":"Save Allocation"}</>}
                </button>
              </div>
            </div>
          </div>
